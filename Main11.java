import java.util.Scanner;

public class Main11 {
  public static void main (String[] args) {
  
    Scanner scanner = new Scanner(System.in);

     double saldo = 200.00;
     double saque;
     double deposito;

    System.out.println("Aperte a tecla l para ver seu saldo \n Aperte a tecla g para fazer um saque \n Aperte a tecla u para fazer um deposito");
   
    String tecla = scanner.nextLine();

    if (tecla.equals( "l")) {
      System.out.println("seu saldo é " + saldo);
    } 
    else if (tecla.equals("g")) {
        System.out.print("qual o valor do saque?");
        saque = scanner.nextDouble();
        scanner.nextLine();
        if (saque < 0.01 || saque > saldo) {
          System.out.println("você não pode sacar menos do que 0,01 ou um valor maior do que tem");
        } else {
        System.out.println("3\n2 \n 1\nsaque completo");
        saldo -= saque;
        System.out.println("seu saldo agora é " + saldo);
       } 
    } 
    else if (tecla.equals("u")) {
        System.out.print("qual o valor do deposito?");
        deposito = scanner.nextDouble();
        if (deposito < 0.01) {
          System.out.println("você não pode depositar menos do que 0,01");
        } 
        else {

          scanner.nextLine();
          System.out.println("3\n2\n1\ndeposito concluido");
          saldo += deposito;
          System.out.println("seu saldo agora é " + saldo);
        }
        } 
        else {
          System.out.println("comando não reconhecido");
    }
    scanner.close();
  }
}